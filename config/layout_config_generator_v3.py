...
def generate_layout_config_v3(
    ...
    Returns:
        layout_dict: The dictionary written to disk (matching layout_models expected format).
    """
    # Parse ASCII block template
    parser = ASCIIParser(ascii_file)
    parsed = parser.parse()
    # parsed expected keys: 'markers', 'waveguides', 'gratings' (same shape as v2)
    ...
    for row in range(num_rows):
        for col in range(blocks_per_row):
            # design center (u, v): keep same convention as earlier generator
            u_center = float(col * block_spacing)
            v_center = float(-1.0 * row * block_spacing)  # negative row->downwards in design coords
            
            # Fiducials: convert marker corner keys to CornerType and Point2D instances
            fiducials: Dict[lm.CornerType, lm.Point2D] = {}
            for corner_key, pos in markers.items():
                try:
                    corner_enum = lm.CornerType(corner_key)
                except ValueError:
                    # If ASCII parser uses different naming, try lowercasing
                    corner_enum = lm.CornerType(corner_key.lower())
                fiducials[corner_enum] = lm.Point2D.from_list([float(pos[0]), float(pos[1])])

            # Waveguides: create Waveguide instances using from_dict (keeps semantics)
            ...
            # Gratings: create Grating instances but only attach those present in the template
            grating_objs: Dict[str, lm.Grating] = {}
            for g in gratings:
                # Determine closest waveguide (same approach as v2)
                ...
                # Only add gratings if closest waveguide is found
    ...
    # Build the final dict to write to JSON using to_dict helpers where available
    output: Dict = {
        "design_name": design_name,
        "version": version,
        "coordinate_system": coordinate_system,
        "block_layout": block_layout.to_dict(),
        # Blocks must be string-keyed in JSON (layout_models expects that when reading)
        "blocks": {str(bid): blk.to_dict() for bid, blk in blocks.items()},
        # Ground truth serialized via its to_dict (will stringify block IDs as keys)
    ...
    if include_metadata:
        output["metadata"] = {
            ...
            "template_file": str(Path(ascii_file).name),
    ...
    # Write JSON with reasonably compact indentation
    with open(out_path, 'w') as f:
        json.dump(output, f, indent=2)

    # Return the dict for programmatic usage / testing
    return output
...
def load_layout_config_v3(config_file: str = "config/mock_layout.json"):
    ...
    Use RuntimeLayout.from_json_file(...) if you want design-only access.
    """
...
def plot_layout_v3(layout_source, output_path: str = "config/mock_layout.png"):
    """
    Visualize layout using the dataclasses (CameraLayout preferred).
    layout_source: either a path to JSON or a CameraLayout instance or a layout dict.
    """
    ...
    # Local corners [bottom_left, bottom_right, top_right, top_left]
    ...
    for bid, block in blocks.items():
        ...
        # Stage polygon (fabricated + global)
        stage_points = []
        for (u_l, v_l) in local_corners:
            u_fab_local, v_fab_local = apply_block_fab_error(bid, float(u_l), float(v_l))
        ...
        for corner_type, pos in block.fiducials.items():
            ...
            label_fab = f"{corner_type.value} (fab)" if corner_type.value not in plotted_labels else ""
            label_stage = f"{corner_type.value} (stage)" if corner_type.value not in plotted_labels else ""
            plotted_labels.add(corner_type.value)
...
if __name__ == "__main__":
    ...
    # Quick test: attempt to load as CameraLayout and RuntimeLayout
    try:
        cam = lm.CameraLayout.from_json_file(output_file)
        print(f"\nLoaded CameraLayout: {cam}")
        rt = lm.RuntimeLayout.from_json_file(output_file)
        print(f"Loaded RuntimeLayout: {rt}")
    except Exception as e:
        print(f"\n⚠️  Warning: Could not instantiate layouts for test: {e}")
    # Optional: plot layout
    plot_layout_v3(layout_source=layout, output_path="config/mock_layout.png")