"""
Filter Stage Control Panel - WITH MULTI-POSITION SWEEP + HCU SLIT CONTROL
...
- HCU slit stage quick controls (Move Away / Move In)
"""
...
class FilterPanelWidget(QWidget):
    """Control panel for filter stage with multi-position sweep and HCU slit control."""
    
    POSITIONS_FILE = "config/saved_positions.json"
    
    def __init__(self, state, signals, filter_controller, hcu_controller=None, parent=None):
        """
        Args:
            state             : SystemState
            signals           : SystemSignals
            filter_controller : FilterController
            hcu_controller    : HCUController | None  — if None, HCU buttons are disabled
        """
        super().__init__(parent)
        self.state  = state
        self.signals = signals
        self.filter = filter_controller
        self.hcu    = hcu_controller          # may be None
        ...
        # Connect sweep completion signal
    ...
    def _on_sweep_finished(self):
        """Re-enable buttons when sweep finishes."""
    ...
    def _init_ui(self):
        ...
        # ====================================================================
        # HCU SLIT CONTROL  (shown only when HCU controller is provided)
        # ====================================================================
        hcu_group  = QGroupBox("🔬 HCU Slit Control")
        hcu_layout = QVBoxLayout()

        connected = self.hcu is not None and self.hcu.is_connected

        if not connected:
            note = QLabel(
                "ℹ️  HCU stage not connected.\n"
                "Connect hardware and restart to enable slit control."
            )
            note.setStyleSheet(
                "QLabel { color: #888; font-style: italic; font-size: 9pt; }"
            )
            note.setWordWrap(True)
            hcu_layout.addWidget(note)
        else:
            tip = QLabel(
                "Move the k-space slit into or out of the beam.\n"
                "Set exact positions in the <b>Stage 2</b> tab."
            )
            tip.setStyleSheet("QLabel { color: #555; font-size: 9pt; }")
            tip.setWordWrap(True)
            hcu_layout.addWidget(tip)

        btn_row = QHBoxLayout()

        self.btn_move_away = QPushButton("📷  Move Away  (full image)")
        self.btn_move_away.setEnabled(connected)
        self.btn_move_away.setToolTip(
            "Move HCU stage to 'open' position so the full image is visible.\n"
            "Configure this position in the Stage 2 tab."
        )
        self.btn_move_away.setStyleSheet(
            "QPushButton { background-color: #4CAF50; color: white; "
            "font-weight: bold; padding: 10px; font-size: 10pt; }"
            "QPushButton:disabled { background-color: #CCCCCC; color: #888; }"
        )
        if connected:
            self.btn_move_away.clicked.connect(self.hcu.move_away)
        btn_row.addWidget(self.btn_move_away)

        self.btn_move_in = QPushButton("🔬  Move In  (slit / filter)")
        self.btn_move_in.setEnabled(connected)
        self.btn_move_in.setToolTip(
            "Move HCU stage to 'slit' position to activate k-space filtering.\n"
            "Configure this position in the Stage 2 tab."
        )
        self.btn_move_in.setStyleSheet(
            "QPushButton { background-color: #9C27B0; color: white; "
            "font-weight: bold; padding: 10px; font-size: 10pt; }"
            "QPushButton:disabled { background-color: #CCCCCC; color: #888; }"
        )
        if connected:
            self.btn_move_in.clicked.connect(self.hcu.move_in)
        btn_row.addWidget(self.btn_move_in)

        hcu_layout.addLayout(btn_row)

        # Live HCU position label (updates via timer)
        self._hcu_pos_label = QLabel("HCU: --")
        self._hcu_pos_label.setStyleSheet(
            "QLabel { font-family: monospace; font-size: 9pt; color: #555; }"
        )
        hcu_layout.addWidget(self._hcu_pos_label)

        hcu_group.setLayout(hcu_layout)
        layout.addWidget(hcu_group)
        ...
        self.position_list.setStyleSheet(
            "QListWidget { background-color: #E0E0E0; } "
            "QListWidget::item:selected { background-color: #B0B0B0; }"
        )
        ...
        self.settle_spin.setRange(0.0, 30.0)
    ...
    # ------------------------------------------------------------------
    # Saved XYZ positions (for multi-sweep)
    # ------------------------------------------------------------------
    ...
    def _refresh_saved_positions(self):
        ...
        try:
            ...
            for name in sorted(positions.keys()):
                ...
                self.position_list.setItemWidget(item, cb)
    ...
    def _select_all_positions(self):
        for i in range(self.position_list.count()):
            w = self.position_list.itemWidget(self.position_list.item(i))
    ...
    def _get_selected_positions(self) -> list:
        ...
        for i in range(self.position_list.count()):
            ...
            w    = self.position_list.itemWidget(item)
            if isinstance(w, QCheckBox) and w.isChecked():
                result.append(item.data(Qt.ItemDataRole.UserRole))
        return result

    # ------------------------------------------------------------------
    # Position display (1D filter + HCU)
    # ------------------------------------------------------------------
    ...
    def _update_position(self):
        ...
        if self.filter.filter_stage is None:
            ...
            try:
                pos_um = self.filter.filter_stage.get_position() / 1000.0
            ...
            try:
                if self.filter.camera and hasattr(self.filter.camera, 'get_exposure_time'):
                    exp_ms = self.filter.camera.get_exposure_time() * 1000
                    self.exposure_label.setText(f"Camera Exposure: {exp_ms:.2f} ms")
                else:
                    self.exposure_label.setText("Camera Exposure: N/A")
            except Exception:
                self.exposure_label.setText("Camera Exposure: Error")
    ...
    def _goto_position(self, checked=False):
        self.filter.move_to_position(self.goto_spin.value())
    ...
    def _update_position_count(self):
        ...
        if step > 0 and end >= start:
            ...
            dmm = (end - start) / 1000
            self.num_positions_label.setText(f"Positions: {n} (over {dmm:.1f} mm)")
        else:
    ...
    def _get_desktop_path(self) -> str:
        ...
        home = P.home()
        for name in ("Desktop", "Рабочий стол", "Schreibtisch"):
            d = home / name
            if d.exists():
                return str(d)
        return str(home)
    ...
    def _browse_output(self):
        ...
        start = self.output_edit.text() or self._get_desktop_path()
    ...
    def _run_single_sweep(self):
        ...
        if self.filter.run_sweep(start_um=start, end_um=end, step_um=step,
                                  output_dir=output, settle_time_s=settle):
    ...
    def _run_multi_sweep(self):
        ...
        if reply != QMessageBox.StandardButton.Yes:
            return
        if self.filter.run_multi_position_sweep(
            positions=selected, start_um=start, end_um=end,
            step_um=step, output_dir=output, settle_time_s=settle
        ):
            self._set_sweep_running(True)
