# ============================================================================
# FILE 1: app/widgets/automated_alignment_panel.py
# ============================================================================
"""
Automated Alignment Panel - Grid search with progress dialogs.
This is the existing implementation, kept as-is.
"""
...
class AutomatedAlignmentPanel(QWidget):
    """Automated alignment using grid search (existing implementation)."""
    ...
    def _init_ui(self):
        ...
        self.btn_run_global = QPushButton("▶️ Run Global Alignment")
        self.btn_run_global.setStyleSheet(
    ...
    def _connect_signals(self):
        ...
        self.signals.global_alignment_complete.connect(self._on_global_complete)
        self.signals.block_alignment_complete.connect(
            lambda bid, res: self._update_block_status(bid)
        )
    ...
    def _run_global(self):
        ...
        self.alignment_controller.start_global_alignment(
            corner_pairs=[(1, 'top_left'), (20, 'bottom_right')],
    ...
    def _on_global_complete(self, results):
        """Update after global calibration."""
    ...
    def _update_global_status(self):
        ...
        if self.state.global_calibrated:
            ...
            if self.state.global_calibration_params:
                ...
                trans = params.get('translation_um', (0, 0))
                err = params.get('mean_error_um', 0)
    ...
    def refresh_status(self):
        """Refresh all status displays (called from other tabs)."""
        self._update_global_status()
        if self.state.navigation.current_block:
            self._update_block_status(self.state.navigation.current_block)
