...
"""Waveguide Navigation Panel - CLEAN VERSION (no 'center' column)."""
...
class WaveguidePanelWidget(QWidget):
    """Waveguide table and navigation controls - without 'center' column."""
    ...
    def _init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            'WG#', 'V-Center (µm)', 'Left', 'Right', 'Status'
        ])
    ...
    def _populate_table(self):
        ...
        for i in range(50):
            ...
            # V-center placeholder
            item = QTableWidgetItem("--")
            item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.table.setItem(i, 1, item)

            # Left & Right buttons
            for col, side in enumerate(['left', 'right'], start=2):
                btn = QPushButton("Go")
    ...
    def _on_block_selected(self, block_id: int):
        """Handle block selection - update table."""
        ...
        if is_calibrated:
            ...
            self.info_label.setStyleSheet("QLabel { color: orange; font-weight: bold; font-style: italic; }")

        self.btn_goto_target.setEnabled(True)
        self.btn_goto_beam.setEnabled(True)
        ...
        try:
            ...
            for i in range(50):
                ...
                if wg_num in available_wgs:
                    ...
                    self.table.item(i, 1).setText(f"{wg.v_center:.2f}")
                    ...
                    for col, side in enumerate(['left', 'right'], start=2):
                        ...
                        if has_grating:
                            btn.setEnabled(True)
                            color = "#4CAF50" if is_calibrated else "#FF9800"
            ...
            self.info_label.setStyleSheet("QLabel { color: red; font-weight: bold; font-style: italic; }")
    ...
    def _goto_waveguide(self, wg_num: int, side: str):
        ...
        if self.state.navigation.current_block is None:
            self.signals.error_occurred.emit(
                "No Block Selected", "Please select a block first"
            )
            return
        ...
        success = self.navigation.navigate_to_grating(
            block_id=block_id, waveguide=wg_num, side=side
        )
        if not success:
            return
    ...
    def navigate_to_beam(self):
        ...
        offset_stage_y = offset_px_x * um_per_pixel
        offset_stage_z = -offset_px_y * um_per_pixel
